...
InlineLexer.prototype.output = function(src) {
  ...
  while (src) {
    ...
    // extended
    if (!this.inLink && this.options.extended && (cap = this.rules.extended.exec(src))) {
      src = src.substring(cap[0].length);
      out += this.renderer.extended(cap[1], cap[2])
      continue;
    }
...
Renderer.prototype.extended = function(preceding, content) {
  var m = content.match(/([^\s]*)\s*(.*)/)
  return (preceding||'') +
    '<span class="ssb-extended" ' +
    'data-type="' + m[1] + '"' +
    (m[2] ? ' data-content="' + m[2].trim() + '"': '') +
    '></span>';
}
